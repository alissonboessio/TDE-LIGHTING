...
#include <cmath>
#include <glm/gtc/constants.hpp>  // glm::pi, glm::two_pi, etc.
...
void Cylinder::init(float radius, float height, int segments) {
    ...
    for (int i = 0; i <= segments; i++) {
        ...
        // normal radial (x,0,z) normalizada = (x/r, 0, z/r)
        float nx = cos(theta);
        float nz = sin(theta);
